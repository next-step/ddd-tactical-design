...
class MenuNameTest {
	...
	void menuNameOfNullOrEmpty(String invalidName) {
		...
		assertThatExceptionOfType(IllegalArgumentException.class)
			.isThrownBy(() -> new MenuName(invalidName, purgomalumClient))
	...
	void menuNameWithProfanity(String nameWithProfanity) {
		// When & Then
		assertThatExceptionOfType(IllegalArgumentException.class)
			.isThrownBy(() -> new MenuName(nameWithProfanity, purgomalumClient))
			.withMessage(MenuName.NAME_WITH_PROFANITY_ERROR);
	}
	...
	void menuName() {
		...
		// When
		MenuName menuName = new MenuName(validName, purgomalumClient);
