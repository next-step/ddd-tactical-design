...
class OrderMenusTest {
    ...
    @Test
    void 주문_메뉴_목록_생성_성공() {
        final OrderMenu orderMenu = new OrderMenu(UUID.randomUUID(), UUID.randomUUID(), new Price(16_000L), true);

        final OrderMenus orderMenus = new OrderMenus(Collections.singletonList(orderMenu));

        assertThat(orderMenus.getOrderMenuByMenuId(orderMenu.getMenuId())).isEqualTo(orderMenu);
    }
    ...
    void 유효성_검증_성공() {
        ...
        try {
            orderMenus.validate(Collections.singletonList(orderMenu.getMenuId()));
    ...
    void 공개되지_않은_메뉴로_인한_유효성_검증_실패() {
        final OrderMenu orderMenu = new OrderMenu(UUID.randomUUID(), UUID.randomUUID(), new Price(16_000L), false);
        final OrderMenus orderMenus = new OrderMenus(Collections.singletonList(orderMenu));

        ThrowableAssert.ThrowingCallable when = () -> orderMenus.validate(Collections.singletonList(orderMenu.getMenuId()));
