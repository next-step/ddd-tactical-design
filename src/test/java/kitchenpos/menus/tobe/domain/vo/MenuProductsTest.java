...
class MenuProductsTest {
    ...
    void 메뉴에_등록된_상품_개수와_실제_상품_개수가_일치해야_한다() {
        // given
        final Product product = new Product("후라이드 치킨", valueOf(20_000));

        final UUID duplicateProductId = product.getId();

        // MenuProduct 개수는 2개이지만, 실제 Product는 1개
        final MenuProduct menuProduct1 = new MenuProduct(product, 20_000, 1, duplicateProductId);
        final MenuProduct menuProduct2 = new MenuProduct(product, 20_000, 1, duplicateProductId);
