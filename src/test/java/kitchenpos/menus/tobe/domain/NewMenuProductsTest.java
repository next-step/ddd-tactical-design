...
class NewMenuProductsTest {
    ...
    @Test
    void create() {
        MenuProducts menuProducts = MenuProducts.create(List.of(NewMenuProduct.create(newProduct1, 1L)));

        assertThat(menuProducts).isEqualTo(MenuProducts.create(List.of(NewMenuProduct.create(newProduct1, 1L))));
    }

    @DisplayName("메뉴상품목록이 비어있으면 예외를 반환한다.")
    @NullAndEmptySource
    @ParameterizedTest
    void create_failed(List<NewMenuProduct> input) {
    ...
    void validateMenuPrice() {
        MenuProducts menuProducts = MenuProducts.create(List.of(
                NewMenuProduct.create(newProduct1, 1L),
                NewMenuProduct.create(newProduct2, 2L)
        ));
