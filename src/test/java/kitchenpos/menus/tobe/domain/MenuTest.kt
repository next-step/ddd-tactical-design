...
private val profanityWordValidator = ProfanityWordValidator(FakePurgomalumClient())

class MenuTest : BehaviorSpec({
    Given("메뉴를 생성할 때") {
        When("메뉴 가격이 상품 금액의 합보다 작은 경우") {
            val menuName = MenuName("양념치킨", profanityWordValidator)
            val menuGroup = MenuGroup("베스트 메뉴")
            val menuProducts = MenuProducts(mutableListOf(메뉴상품_1000원, 메뉴상품_2000원))

            Then("예외가 발생한다") {
                shouldThrow<IllegalArgumentException> {
                    Menu(menuName, 2999.won, menuGroup, menuProducts)
                }
            }
        }
        ...
        When("메뉴 가격이 상품 금액과 같거나 크면") {
            ...
            Then("메뉴가 생성된다") {
                val menu = Menu(menuName, 3001.won, menuGroup, menuProducts)

                menu.name shouldBe menuName
