...
public class Fixtures {
    ...
    public static MenuProduct menuProduct(final kitchenpos.products.domain.Product product, final long quantity) {
        final MenuProduct menuProduct = new MenuProduct();
        menuProduct.setSeq(new Random().nextLong());
        menuProduct.setProduct(product);
	...
	public static kitchenpos.products.domain.Product product() {
		return product("후라이드", 16_000L);
	}

	public static kitchenpos.products.domain.Product product(final String name, final long price) {
		final kitchenpos.products.domain.Product product = new kitchenpos.products.domain.Product();
		Product.create(new Name(name, purgomalumClient), new Price(BigDecimal.valueOf(price)));
		return product;
	}

    public static Product tobeProduct() {
        return tobeProduct("후라이드", 16_000L);
    }
