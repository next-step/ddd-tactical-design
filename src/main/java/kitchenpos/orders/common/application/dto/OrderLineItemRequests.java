...
public class OrderLineItemRequests {
    ...
    public List<OrderLineItem> toOrderLineItems(List<Menu> menus, OrderType orderType) {
        return orderLineItems.stream().map(orderLineItem -> orderLineItem.toStore(findMenu(menus, orderLineItem), orderType)).toList();
    }

    private Menu findMenu(List<Menu> menus, OrderLineItemRequest orderLineItem) {
        return menus.stream().filter(menu -> menu.getId().equals(orderLineItem.getMenuId())).findFirst().orElseThrow((NoSuchElementException::new));
    }
}
