...
- **상품을 등록할 수 있다.**
  - [x] 상품의 가격이 존재해야 하며 0보다 커야 한다. 만약 그렇지 않으면 예외가 발생한다.
  - [x] 상품의 이름이 없거나 부적절한 단어(비속어)가 포함되면 예외가 발생한다.

- **상품의 가격을 변경할 수 있다.**
  - [x] 상품의 가격이 존재해야 하며 0보다 커야 한다. 만약 그렇지 않으면 예외가 발생한다.
  - [x] 상품에 대한 ID가 존재해야 한다.
  - [x] 메뉴에 포함된 상품들의 총 가격(sum, 상품 가격 * 수량)은 메뉴 가격보다 작으면 안 된다.
...
- **메뉴 그룹을 등록한다.**
  - [x] 메뉴 그룹명이 존재해야 한다. 만약 메뉴 그룹명이 없거나 빈 값이면 예외가 발생한다.
...
- **메뉴를 등록할 수 있다.**
  ...
  - [x] 메뉴 이름 검증
    - 메뉴 이름이 존재해야 한다. 만약 메뉴 이름이 `null`, 빈 값(`""`), 공백 문자(`" "`)이면 예외가 발생한다.
    - 메뉴 이름에는 비속어가 포함되면 예외가 발생한다.

- **메뉴에 있는 가격을 변경할 수 있다.**
  - [x] 메뉴 가격 검증 - 메뉴 가격을 변경할 때 가격이 0원 이상이어야 한다. 만약 그렇지 않으면 예외가 발생한다.
  - [x] 상품 총 가격 검증 - 메뉴 가격은 포함된 상품들의 총 가격(각 상품 가격 × 수량)의 합보다 클 수 없다. 메뉴 가격이 상품 총 가격보다 크면 예외가 발생한다.

- **메뉴를 표시 상태로 변경할 수 있다.**
  - [x] 메뉴가 존재해야 한다. 만약 메뉴가 존재하지 않으면 예외가 발생한다.
  - [x] 메뉴를 표시하려면 표시 상태(displayed)가 `true`로 변경된다.
  - [x] 메뉴를 표시하려면 메뉴 가격이 포함된 상품들의 총 가격(각 상품 가격 × 수량)의 합보다 클 수 없다. 메뉴 가격이 상품들의 총 가격보다 크면 예외가 발생한다.

- **등록된 메뉴를 숨길 수 있다.**
  - [x] 메뉴가 존재해야 한다. 만약 메뉴가 존재하지 않으면 예외가 발생한다.
  - [x] 메뉴를 숨기기 위해 표시 상태(displayed)가 `false`로 변경된다.
...
- **주문을 등록할 수 있다.**
  ...
  - [x] 주문 유형이 `매장 내 식사`인 경우 주문 수량이 0일 수도 있다. (고객이 미리 자리에 앉아 있고, 아직 메뉴를 정하지 않는 상태에서 자리만 확보할 수 있기 때문)
  - [x] 주문 항목과 연관된 메뉴가 존재해야 한다.
  - [x] 메뉴가 표시되어야(노출 상태이면) 주문이 가능하다.
  - [x] 주문 항목에 있는 가격이 메뉴에 있는 가격과 동일해야 한다.
  - [ ] 주문 유형이 `배달`이면 배달에 필요한 주소가 존재해야 한다.
  - [x] 주문 유형이 `매장 내 식사`이면  손님이 착석한 테이블에서만 주문할 수 있다.

- **주문을 승인할 수 있다.**
  - [x] 주문이 존재해야 한다.
  - [x] 주문 상태가 `주문 대기` 여야 한다.
  - [ ] 주문 상태가 `배달중` 이면 배달 라이더에게 주문에 대한 정보(주문 총 가격, 배달 주소)를 제공해줘야 한다.

- **주문을 서빙할 수 있다.**
  - [x] 주문이 존재해야 한다.
  - [x] 주문 상태가 `주문 접수` 여야 한다.
  - [x] 주문 상태를 `서빙 완료` 로 변경한다.
...
- **주문을 완료할 수 있다.**
  - [x] 주문이 존재해야 한다.
  - [ ] 주문 유형이 `배달` 이면 주문 상태가 `배달 완료` 여야 하며, 이후 `주문 완료` 상태로 전이된다.
  - [ ] 주문 유형이 `포장` 이면 주문 상태가 `서빙 완료` 여야 하며, 이후 `주문 완료` 상태로 전이된다.
  - [x] 주문 유형이 `매장 내 식사` 이면 주문 상태가 `서빙 완료` 여야 하며, 이후 `주문 완료` 상태로 전이된다.
  - [x] 주문 유형이 `매장 내 식사` 이고 해당 테이블에 진행 중인 주문이 하나도 없다면 손님 수를 0명으로 하고, 자리가 비어있다고 변경한다.
...
- **주문 테이블을 등록할 수 있다.**
  - [x] 주문 테이블의 이름이 없으면 등록할 수 없다.
  - [x] 손님 수가 0명이고 테이블 사용 여부가 `사용 가능` 상태여야 한다.

- **주문 테이블에 손님이 앉으면 사용중 상태로 변경한다.**
  - [x] 주문 테이블이 존재해야 한다.
  - [x] 테이블 사용 여부를 `사용중` 으로 변경된다.

- **주문 테이블에 있는 손님 수를 변경할 수 있다.**
  - [x] 손님 수가 `0`명 이상이어야 한다. 0명 미만이면 예외가 발생한다.
  - [x] 주문 테이블이 존재해야 한다.
  - [x] 테이블 사용 여부가 `사용중`(true) 이어야 한다.
  - [x] 위의 조건을 모두 만족하면 해당 손님 수를 변경한다.
...
> 주문 테이블(EatInOrderTable)

| 한글명      | 영문명            | 설명                                                                                                                                     |
|----------|----------------|----------------------------------------------------------------------------------------------------------------------------------------|
| 주문 테이블   | EatInOrderTable     | 매장 내 식사하는 손님을 위한 테이블                                                                                                                   |
| 테이블 이름   | name           | 테이블 이름                                                                                                                                 |
...
| 한글명        | 영문명           | 설명                                                                                                                                    |
|------------|---------------|---------------------------------------------------------------------------------------------------------------------------------------|
| 포장 주문      | TakeOutOrder  | 포장하는 고객 대상. 고객이 매장에서 직접 음식을 수령한다.                                                                                              |
...
| 한글명        | 영문명            | 설명                                                                                                                                                                      |
|------------|----------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 배달 주문      | DeliveryOrder  | 배달하는 고객 대상. 집이나 직장 등 고객이 선택한 주소로 음식을 배달한다.                                                                                                                                      |
...
- 상품을 등록할 수 있다.
  ...
  - 상품의 이름이 존재하지 않거나 부적절한 단어(비속어)가 포함되면 인된다.
...
#### 속성(상태)
- 메뉴(`Menu`)
  - 메뉴(`Menu`)에는 메뉴 이름(`MenuName`), 메뉴 가격(`MenuPrice`), 메뉴 노출 여부(`displayed`) 가 있다.
...
- 주문을 등록할 수 있다.
  ...
  - 주문 항목이 존재해야 한다.
  - 주문 유형이 `배달` 혹은 `포장`시 주문 항목에 있는 수량이 0보다 커야 한다.
  - 주문 유형이 `매장 내 식사`인 경우 주문 수량이 0일 수도 있다. (고객이 미리 자리에 앉아 있고, 아직 메뉴를 정하지 않는 상태에서 자리만 확보할 수 있기 때문)
  - 주문 항목과 연관된 메뉴가 존재해야 한다.
  - 메뉴가 표시되어야(노출 상태이면) 주문이 가능하다.
  - 주문 항목에 있는 가격이 메뉴에 있는 가격과 동일해야 한다.
  - 주문 유형이 `배달`이면 배달에 필요한 주소가 존재해야 한다.
  - 주문 유형이 `매장 내 식사`이면  손님이 착석한 테이블에서만 주문할 수 있다.
...
- 주문을 완료한다.
  ...
  - 주문 유형이 `배달` 이면 주문 상태가 `배달 완료` 여야 하며, 이후 `주문 완료` 상태로 전이된다.
  - 주문 유형이 `포장` 이면 주문 상태가 `서빙 완료` 여야 하며, 이후 `주문 완료` 상태로 전이된다.
  - 주문 유형이 `매장 내 식사` 이면 주문 상태가 `서빙 완료` 여야 하며, 이후 `주문 완료` 상태로 전이된다.
