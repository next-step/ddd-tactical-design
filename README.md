...
- 상품을 등록할 수 있다.
  - [x] 가격이 상품가격정책에 부합하지 않은 상품은 등록할 수 없다
  - [x] 이름이 상품이름정책에 부합하지 않은 상품은 등록할 수 없다
- [x] 상품의 가격을 변경할 수 있다.
  - [x] 상품가격정책에 부합하지 않은 가격으로는 변경할 수 없다
...
| 상품가격정책 | ProductPricing Rule | 상품의 가격은 0원 이상어야 한다 |
| 상품이름정책 | ProductNaming Rule | 상품의 이름에는 비속어가 포함될 수 없다 |
