...
- `메뉴 가격`이 변경될 때 `메뉴 가격`이 `메뉴 상품들`의 `총액`보다 크면 메뉴가 숨겨진다.
- `메뉴상품들` 중에 `상품`에 존재하지 않는 `상품`이 있으면 `메뉴`를 생성할 수 없습니다.
- 메뉴를 숨길 수 있다.
- 메뉴의 목록을 조회할 수 있다.
...
- 고객 인원를 변경할 수 있다.
- 고객 인원이 올바르지 않으면 변경할 수 없다.
  - 고객 인원는 0 이상이어야 한다.
- 빈 테이블은 방문한 고객 인원을 변경할 수 없다.
...
| 주문 메뉴 | OrderLine Item   | 주문을 구성하는 하위 메뉴 항목들                  |
...
- 행동
  ...
  3. `사장님`이 `메뉴 가격`을 변경할 수 있다.
    - `메뉴 상품 가격`의 총액보다 `메뉴 가격`이 클 수 없다.
...
- 행동
  1. `주문 종류`: 공통 (해당 조건은 공통으로 지켜져야 한다.)
    - `주문`을 생성할 때, `대기중(WAITING)` 으로 `주문 상태`를 변경한다.
    - `메뉴` 내의 `주문 메뉴`들이 삭제된 `상품`이면 안된다.
    - `메뉴`가 생성된 상태이고 `공개 상태(VISIBLE)` 되어있어야한다.
    - `주문 가격` 총액과 `주문`에 속한 `메뉴 가격` 총액이 동일해야한다.
    - `주문`은 `메뉴` 1개 이상으로 구성되어야 한다.
  2. `주문 종류`: `배달주문`
    - `주문`이 생성할 때, `주문 메뉴`들의 `갯수`를 감소 시킬 수 없다.
