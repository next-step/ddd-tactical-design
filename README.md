...
### 매장 주문
#### 속성
- `OrderTable`은 식별자, `name`, `NumberOfGuests`, `OrderTableStatus`를 가진다.
- `EatInOrder`는 식별자, 주문시간, `OrderStatus`, `OrderLineItems`, `OrderTableId`를 가진다.
- `OrderLineItem`은 `seq`, `quantity`, `menuId`를 가진다.

#### OrderTable 기능
- `OrderTable` 생성
  - `OrderTable`의 `name`은 필수값이다.
  - `OrderTable`생성 시 초기값은 `EmptyTable`이다.
- `OrderTable`의 `NumberOfGuests` 변경
  - `NumberOfGuests`는 0 이상이어야 한다.
  - `EmptyTable`인 경우 `NumberOfGuests`는 변경할 수 없다.
- `OrderTable`의 `EmptyTable` 설정
  - `Complete`되지 않은 `EatInOrder`가 있는 경우 `EmptyTable`로 설정할 수 없다.
- `OrderTable`의 `OccupiedTable` 설정

#### EatInOrder 기능
- `EatInOrder` 생성
  - `EatInOrder`의 `OrderLineItems`는 1개 이상이어야 한다.
  - `EatInOrder`의 `OrderLineItems`의 `menu`는 `Displayed`여야 한다.
  - `EatInOrder`는 `OccupiedTable`을 1개 가져야한다.
  - `EatInOrder`생성 시 `OrderStatus`는 `Waiting`이다.
- `EatInOrder` 접수
  - `EatInOrder`는 `Waiting`일떄만 `Accept`할 수 있다.
- `EatInOrder` 서빙
  - `EatInOrder`는 `Accepted`일때만 `Served`할 수 있다.
- `EatInOrder` 완료
  - `EatInOrder`는 `Served`일때만 `Complete`할 수 있다.
  - `EatInOrder`이 가진 `OrderTable`의 모든 `EatInOrder`가 `Complete`되면 `EmptyTable`이 된다.
