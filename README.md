...
- 주문 테이블은 식별자(`id`)와 이름(`DisplayedName`), 방문한 손님 수(`NumberOfGuests`), 빈 테이블 여부(`isEmpty`)를 가진다.
- 주문 테이블이 빈 테이블이면, 주문을 추가할 수 없다.
- 주문 테이블은 빈 테이블로 설정(`sit`)된다.
- 주문 테이블에 계산 완료(`Completed`)되지 않은 주문이 있으면, 빈 테이블로 설정되지 않는다.
...
- 주문은 등록된 주문 테이블의 식별자가 필수다.
- 주문은 주문 테이블 식별자가 빈 테이블의 것이 아니어야 한다.
- 주문은 주문 항목 목록(`OrderLineItems`)이 필수다.
- 주문은 생성될 때, 주문 상태는 접수 대기(`Waiting`)다.
- 주문의 주문 상태는 다음 단계로 진행(`proceed`)된다.
- 주문이 계산 완료되면, 주문 테이블의 방문한 손님 수(`NumberOfGuests`)를 0으로 변경하고, 빈 테이블로 설정한다.
